...
--------------------------------
The program makes use of strings, string streams and file streams in C++ to read and store values into a full fledged 2D array. This array is dynamically allocated memory and is handled throughout by reference. Apart from the Main() function, I am making use of two other functions namely:
...
4) write the value and associated indices of the value to localPeaks_Output.txt
...
---------------------
Apart from the various console messages, the "findPeaks_Output.txt" file is saved in the same folder as that of the program.

What works? 
- Reading from input FILE and converting it to an array.
- Finding local peaks and writing to an output FILE.

What doesn't work?
- Doesn't consider boundary elements in the matrix. The program does not find peaks in the boundary.
